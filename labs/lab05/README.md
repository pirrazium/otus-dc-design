...
* Считается не количество сегментов, а количество хопов, p2p линк — это не дополнительный хоп, а часть пути, который BGP видит как один IP-прыжок.
...
* Оставляю для будущих вебинаров, там будет объяснение, в крайнем случае переспрошу.
...
- Если хватит ресурсов, то надо попробовать добавить OSPF+iBGP и OSPF+eBGP с уникальными AS, для изучения как работают allowas-in и best-path relax, а так же посмотреть, что будет, если их не будет. Если ресурсов не хватит, то заводить как отдельные лабы.
