...
С опциональным заданием есть небольшая проблема, связанная с тем, что устройства не фиксируют падение "физических" линков, в случае выключения линков со стороны leaf'ов, да и в случае выключения лифа в лабе тоже. Соответственно что лифы, что устройства продолжают кидать часть пакетов в выключенный линк, который горит активным.
Если руками выключать с двух сторон, то все ок, маршруты пропадают. 

Вопросы : 

1. Со стороны мультихоминга настраиваем link tracking, который фиксирует падение аплинков и выключает нижестоящие линки, но для MLAG как-то непонятно как работает этот механизм, в случае отказа аплинков на одном лифе и даунлинков на другом, трафик начнет кататься по peer-link через STP, и поэтому нам в целом не нужно трекать этот момент вовсе. 
...
Сходимость информации о падении MLAG соседа дотюнивать надо или нам вообще это не важно, потому что маршрутов от него в фабрике нет и он не принимает участие в передаче трафика ? Зачем тогда нам знать кто мастер, а кто слейв в VPC паре ? 

В целом VPC в EVPN как-то не до конца понял получается, даже с пересмотром вебинара.  
Можете ссылок на статьи подкинуть под EVPN VPC ? 