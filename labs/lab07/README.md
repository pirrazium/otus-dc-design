...
1. Для MLAG как-то непонятно работает механизм разворачивания VxLAN, в случае отказа аплинков на одном лифе и даунлинков на другом, трафик с src Host_1 начнет кататься по peer-link в виде чистых L2 пакетов, без VxLAN, я так понимаю дальше он доходит до vPC соседа, который оборачивает L2 в VxLAN и отправляет в фабрику, пакеты с dst Host_1 летят в этом же peer-link уже с меткой VXLAN, между MLAG соседями и конечным хостом влан растянут в кольце. 
